$( function() {
  ...
  $( '.groupon_form' ).submit( function( evt ) {
    if ($('#subscription_submit').is('.disabled')) {
      evt.preventDefault();
      ...
      evt.preventDefault();
      evt.stopImmediatePropagation();
    } else if (typeof Groupon.CookieCutter != 'undefined') {
      Groupon.CookieCutter.set_cookie('visited', 'true', 365);
    }
  });
  ...
  function forward() {
    var promptingField = $('.form_step.'+getSteps().curr).find('.prompting_field');

    if (promptingField.length == 0 || (promptingField.length >= 1 && promptingField.valid())) {
      if ( !animating ) {
        animating = true;
        currentStep++;
        styleSteps( 1000 );
