...
<?php
	...
	$sql_previous = "SELECT * FROM ".TABLE_DEALS." WHERE is_previous = 'y' AND city != '$city' AND deal_end_time < '".date("Y-m-d H:i:s")."' LIMIT 0, 30";		// deal_end_time >= '".date("Y-m-d")."'
...
<div style="height: 38px;"><p><a href="<?php echo SITE_URL; ?>?action=sold&id=<?php echo $previous_res['deal_id']; ?>"><?php echo truncate_string($previous_details['title'], 100); ?></a></p></div>
...
<p><?php echo ($previous_buy[0] == ''? '1' : $previous_buy[0]); ?></p>
...
<?php } 	// End while ?>
<?php } else { ?>
<h3>There is no Previous deal</h3>
<?php } ?>
<!-- deal box ends -->
